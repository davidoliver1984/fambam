...
class InvitationLifecycleTest extends TestCase
{
    ...
    public function test_operator_can_bootstrap_only_the_first_account(): void
    {
        ...
        $this->assertTrue($owner->can_invite);
    ...
    public function test_only_invitation_capable_accounts_can_manage_invitations(): void
    {
        $ordinaryUser = User::factory()->create();

        $this->actingAs($ordinaryUser)
            ->postJson('/api/invitations', ['email' => 'relative@example.test'])
            ->assertForbidden();
    ...
    {
        $owner = User::factory()->canInvite()->create();
    ...
    public function test_exchange_consumes_the_email_token_and_acceptance_creates_a_verified_account(): void
    {
        ...
        $this->assertFalse($relative->can_invite);
    ...
    public function test_existing_account_and_duplicate_pending_invitation_are_rejected(): void
    {
        $owner = User::factory()->canInvite()->create();
        User::factory()->create(['email' => 'member@example.test']);

        $this->actingAs($owner)
            ->postJson('/api/invitations', ['email' => 'member@example.test'])
            ->assertUnprocessable();
    ...
    private function issueInvitation(User $owner, string $email): string
    {
        ...
        $this->actingAs($owner)
            ...
            ->postJson('/api/invitations', ['email' => $email])
            ->assertCreated();
