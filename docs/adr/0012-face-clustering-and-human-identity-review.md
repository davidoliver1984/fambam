...
- **`face_identity_assignments.person_id`** naming the absorbed Person is
  ...
  `rejected`/`withdrawn` row is not active by definition. **A still-
  `pending` row is instead transitioned to `withdrawn` (§3), never
  repointed.** The reason is **not** a uniqueness conflict — the
  constraint is keyed by `FaceObservation`, not by Person, so a bare
  repoint would not itself violate it, and an earlier draft's framing to
  the contrary was wrong. The real reason is semantic: a `pending`
  assignment records an *unvalidated machine opinion*, generated by
  comparing one `FaceObservation` against one specific Person's trusted
  gallery (§6) at the moment it was created. Once that Person is merged
  away, the gallery it was actually compared against no longer exists as
  an independent thing — silently repointing the row would assert a
  machine opinion about a comparison ("is this the survivor?") that was
  never actually run. Merge semantics require deliberately retiring that
  now-stale opinion, not quietly relabelling it as if it had always been
  about the survivor. Both paths are captured in, and restorable from,
  the merge's existing snapshot exactly like every other reconciled
  table.
...
- **No Phase 10 structure may retain a foreign key to the absorbed Person
  ...
  reference in an ordinary table; the `FaceIdentitySuppression` collision
  case above is the one place this reconciliation had to correct that
...
- **Treating a Person-merge-driven pending-assignment retirement as a
  ...
  actually exists. The real reason a pending assignment is `withdrawn`
  rather than repointed is semantic — it represents a machine opinion
  computed against a gallery that no longer independently exists — not a
...
- If a merge-driven pending-assignment retirement is ever implemented as a
  repoint "for simplicity" rather than a `withdrawn` transition, a stale,
  never-actually-computed machine opinion could survive the merge and be
  presented as though it meant something about the survivor — worth a
  direct test asserting a pending `face_identity_assignments` row naming
  the absorbed Person is `withdrawn`, never repointed, after merge.
...
- **`FPA-P10-S05`** implements §8 and §9's merge/split/naming-resolves-to-
  ...
  (status-dependent repoint-vs-`withdrawn` handling) and
  `FaceIdentitySuppression` (repoint, or delete-with-snapshot on
  collision, per §12's corrected rule), since both tables must exist
...
- **Required regression tests**: (1) two `FaceObservation`s of the same
  ...
  Persons must repoint `approved`/`rejected`/`withdrawn`
  `face_identity_assignments` unconditionally, must transition a
  `pending` `face_identity_assignments` row to `withdrawn` rather than
  repointing it, and must apply §12's corrected collision rule for
  `FaceIdentitySuppression` exactly (repoint when no collision exists;
  ...
  still references the absorbed Person's id); (14) reversing that merge
  must restore Phase 10 state to exactly its pre-merge shape, including
  reinserting any row §12's collision rule deleted, using the extended
  `captureState`/`restoreState` mechanism; (15) no automatic process may
...
12. **Person-merge integration** — extends `PersonMergeManager`'s existing
    ...
    `withdrawn` `face_identity_assignments` repoint unconditionally; a
    `pending` `face_identity_assignments` row is transitioned to
    `withdrawn` instead, since it represents an unvalidated machine
    opinion computed against a gallery that no longer independently
    exists, not because of any uniqueness conflict; `FaceIdentitySuppression`
    repoints where no collision exists, or is deleted from the live table
    and preserved only in the merge snapshot where a survivor-side row
    already exists — **no Phase 10 live structure ever retains a foreign
    key to the absorbed Person after merge, without exception**; clusters
    need no reconciliation; guarded automatic reversal, including
    reinserting any snapshot-preserved row, is inherited for free.
